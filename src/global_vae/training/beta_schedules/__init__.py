...
(`constant`, `linear_warmup`) via their `@registerBetaSchedule`
decorators. A `@registerX(...)` decorator only runs once its module is
imported; without these imports, `getBetaScheduleClass("constant")`
would raise `KeyError` even though `constant.py` exists on disk.
"""
